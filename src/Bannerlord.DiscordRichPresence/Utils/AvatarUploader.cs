...
namespace Bannerlord.DiscordRichPresence.Utils
{
    ...
    {
        public static async void UploadCharacterTexture(Texture characterTexture)
        {
        ...
        /*
        public static async void UploadBannerTexture(Texture bannerTexture)
        {
            const string localPath = "discord_rpc_banner_temp.bin";
            // Credits to billw2012, taken from
            // https://github.com/billw2012/Bannerlord-Twitch/blob/54e117b97b39476f35adefd23a1bb9e0f06d09c2/BannerlordTwitch/BannerlordTwitch/Documentation/DocumentationGenerator.cs#L329-L334
            bannerTexture.TransformRenderTargetToResource(localPath);
            bannerTexture.SaveToFile(localPath);
            for (var i = 0; i < 100 && !File.Exists(localPath); i++)
                await Task.Delay(100);

            DiscordSubModule.Instance.BannerUrl = await UploadAvatarAsync(File.ReadAllBytes(localPath));
            //File.Delete(localPath);
        }
        */
