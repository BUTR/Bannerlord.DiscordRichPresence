...
For ($i = 0; $i -le $gameversions.Length - 1; $i++)
{
    ...
    msbuild $proj /t:Clean /p:configuration=Release;
    msbuild $proj /p:configuration=Release /p:OverrideGameVersion=$gameversion /p:GameFolder="$OutputPath" /p:ExtendedBuild=false;
